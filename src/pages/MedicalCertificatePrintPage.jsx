...
export default function MedicalCertificatePrintPage() {
  ...
  const purpose = searchParams.get("purpose") || "Attestation médicale";
        ...
        <div className="mx-auto max-w-4xl rounded-3xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
          ...
          <div className="space-y-8 text-sm leading-7 text-slate-800">
            <section className="rounded-2xl border border-slate-200 p-5">
              <p>
                Je soussigné(e), {profile?.full_name || "Médecin"}, certifie avoir examiné ce jour :
              </p>

              <p className="mt-5 font-semibold">{patientName || "—"}</p>

              <p className="mt-3">Date de naissance : {patient.date_naissance || "—"}</p>
              <p className="mt-1">Téléphone : {patient.telephone || "—"}</p>

              <p className="mt-6">
                et atteste que l’état de santé justifie :
              </p>

              <p className="mt-4 font-semibold">{purpose}</p>

              <p className="mt-4">
                à compter du <span className="font-semibold">{formatDate(start)}</span>
                {days ? (
                  <>
                    {" "}pour une durée de{" "}
              ...
              <div className="mt-6 whitespace-pre-wrap">
                {details || "Certificat délivré à la demande de l’intéressé(e) pour servir et valoir ce que de droit."}
              </div>

              {profile?.certificate_signoff ? (
